<script lang="ts">
    ...
    let coins:any;
    let coin_ohlc_data:{[key:number]:{id:string,name:string,symbol:string}[]}={};
    let coin_data:any =[]
          ...
          <div class="p-4">
            ...
            <div class="flex flex-col">
              ...
              <input  on:change={async (event)=>{
                if (event.target instanceof HTMLInputElement) {
                  purchase_date= new Date(event.target.value).toISOString().split('T')[0]
                }
                let selected_row;
                let table = await get_coin_ohlc(i)
                selected_row = table.find((row)=>new Date(row[0]).toISOString().split("T")[0]===purchase_date)
            ...
            <div class=" flex justify-between p-4">
              ...
              <button class="bg-green-500 text-white p-2 rounded"
                on:click={()=>{
                  portafolio.push({id:coin.id,name:coin.name,precio_compra:purchase_price[i],precio_actual:coin_data[i].market_data.current_price.usd,cantidad_de_compra:purchase_ammount[i],fecha_de_compra:})
                }}
  ...
  <div class="bg-gray-200 rounded p-4">
    {#each portafolio as coin}
      <div>
        Id:{coin.id}
        Name:{coin.name}
        Cantidad Comprada:{coin.cantidad_de_compra}
        Precio de Compra:{coin.precio_compra}
        Precio Actual:{coin.precio_actual}
        Inversion Total:{coin.precio_compra*coin.cantidad_de_compra}
        Valor actual del protafolio:{coin.precio_actual*coin.cantidad_de_compra}
        Profit/Loss:{coin.precio_actual*coin.cantidad_de_compra-coin.precio_compra*coin.cantidad_de_compra}
        ROI:{(coin.precio_actual*coin.cantidad_de_compra-coin.precio_compra*coin.cantidad_de_compra)/coin.precio_compra*coin.cantidad_de_compra}
