<script lang="ts">
    ...
    $: graficar
    $: purchase_date, async ()=>{
      let date = new Date(purchase_date).toISOString().split('T')[0]
      let row = await get_coin_ohlc(current_index)
      console.log(row)
      //purchase_price[current_index]= 
    }
  </script>
...
<div class="m-4 p-4 bg-gray-200 rounded">
      ...
      {#if open_data[i]}
        <div class="flex p-4">
          <img src={coin_data[i].image.small} alt="img" />
            <p class="p-2">{coin_data[i].description.en.substring(0,255)+"..."}</p>
        </div>
        <div>
          <div>
            <p>Precio Actual: {coin_data[i].market_data.current_price.usd}</p>

          </div>
          <form class="p-4">
            <div class="flex flex-col">
              <label for="Cantidad">Cantidad a comprar</label>
              <input class="bg-white border border-black rounded" placeholder="Cantidad a comprar" type="number" id="Cantidad"/>
            </div>
            <div class="flex flex-col">
              <label for="Compra">Fecha de comprar</label>
              <input bind:value={purchase_date} on:change={()=>current_index=i} class="bg-white border border-black rounded" placeholder="Fecha de comprar" type="date" id="Compra"/>
            </div>
  ...
  {:else}
      <div>
        <button on:click={()=>{graficar=undefined, console.log(graficar)}} class="bg-red-500 m-2 text-white p-2 rounded">Regresar</button>
