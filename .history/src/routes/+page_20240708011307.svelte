...
{#if portafolio.length!=0}
  ...
  <div>
    ...
    <p>Por motivos de simpicidad aqui se realizaran los calculos considerando exactametne un año de informacion </p>
    <div>
      <table class="m-2 border border-black rounded">
        <thead>
          {#each portafolio as data }
            <th>{data.name}</th>
          {/each}
        </thead>
        <tbody>
          {#each Object.values(coin_ohlc_data)[0] as _,i}
            <tr>
